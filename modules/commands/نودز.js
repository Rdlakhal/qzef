...
module.exports.config = {
  ...
  description: "جميل جدًا وممتع للعين (ملاحظة ، هذا الامر 18+ ضع في اعتبارك قبل الاستخدام:)",
...
"https://i.imgur.com/QYGOZuK.jpg",
  ...
  var money = data.money
      if(money < 300) api.sendMessage("تحتاج 300 دولار لرؤية الصورة",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 300})
   var callback = () => api.sendMessage({body:`سينباي سيعجبه هذا😼\n» الثمن: -300$ «`,attachment: fs.createReadStream(__dirname + "/cache/5.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/5.jpg"), event.messageID); 
