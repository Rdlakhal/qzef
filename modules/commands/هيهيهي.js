...
var min = Math.floor(Math.random() * 1);
  ...
  var money = data.money
      if(money < 800) api.sendMessage("800$ يا سينباي ",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 800})
   var callback = () => 
   api.sendMessage({body:`الثمن: 800$`,attachment: fs.createReadStream(__dirname + "/cache/1.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), event.messageID);
